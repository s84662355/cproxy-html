        ...
        <div style="display: flex;flex-direction: row">
          ...
          <div class="context_form">
            <div class="register_title">
              {{ $t('loginModifyRegister.titleForgotPassword') }}
            </div>
...
import {SendEmailCode,ForgetPassword} from "@/utils/api.js";
...
export default {
  ...
  methods: {
    handleGetCode(){
          if(new Date().getTime()-this.sendEmailCodeTime<10){
                  this.$message({
                      message: "等一会再发",
                      type: 'warning',
                      duration:1000
          ...
          this.sendEmailCodeTime=new Date().getTime()
          const that=this

          SendEmailCode(this.form.email,2,function(message){
              console.log(message)
              if (message.error) {
                    that.$message({
    ...
    handleSubmit(){
      this.$refs.formRef.validate((valid) => {
        ...
        const that=this
        ForgetPassword(this.form.email,this.form.password,this.form.verificationCode,function(message){
              if (message.error) {
                    that.$message({
                        message: message.data,
                        type: 'warning',
